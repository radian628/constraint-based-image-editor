(() => {
  ...
  function keybind(keymap, event) {
    const keymapEntries = Object.entries(keymap);
    for (const [name, keybinds] of keymapEntries) {
      for (const kb of keybinds) {
        if (kb.key.toLowerCase() === event.key.toLowerCase() && !!kb.ctrl === !!event.ctrlKey && !!kb.shift === !!event.shiftKey && !!kb.alt === !!event.altKey) {
          return name;
        }
      }
    }
  ...
  // src/utils/match.tsx
  function match(obj, defaultValue) {
    return (k) => {
  ...
  function PanelSet(props) {
    ...
    function movePanel(i, dx) {
      setTempOffsets((tempOffsets2) => ({
        ...tempOffsets2,
        [i]: (tempOffsets2[i] ?? 0) + dx
      }));
    }
    ...
    function getDisplayTempOffset(i) {
      const rawOffset = tempOffsets[i];
      if (!rawOffset)
        return 0;
      if (-rawOffset >= props.subpanels[i][0])
        return -props.subpanels[i][0];
      if (rawOffset >= props.subpanels[i + 1][0])
        return props.subpanels[i + 1][0];
      return rawOffset;
    }
    const dir = props.vertical ? "vertical" : "horizontal";
    return /* @__PURE__ */ import_react.default.createElement("div", { className: `${dir}-panel`, ref: rootElemRef }, props.subpanels.map(([subdiv, subpanel], i) => {
      const size = `${Math.max(
        0.01,
      ...
      return /* @__PURE__ */ import_react.default.createElement(
        "div",
        {
          ...
          style: {
            width: props.vertical ? void 0 : size,
            height: props.vertical ? size : void 0
          }
        },
        /* @__PURE__ */ import_react.default.createElement("div", { className: `${dir}-subpanel-content` }, subpanel),
        i < props.subpanels.length - 1 && /* @__PURE__ */ import_react.default.createElement(
          "div",
          {
            tabIndex: 0,
            onKeyDown: (e) => {
            ...
            onMouseDown: (e) => {
              ...
              if (e.button === 0) {
                ...
                const onMouseMove = (e2) => {
                  const dx = (props.vertical ? e2.clientY : e2.clientX) - x;
                  const rect = rootElemRef.current?.getBoundingClientRect();
                  const dxN = dx / ((props.vertical ? rect?.height : rect?.width) ?? Infinity);
                  movePanel(i, dxN);
                  x = props.vertical ? e2.clientY : e2.clientX;
                };
                const onMouseUp = (e2) => {
